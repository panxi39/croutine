...
int croutine_worker_init(struct croutine_worker *worker,
	...
	worker->local_queue =
		croutine_cldeque_init(scheduler->config.local_queue_capacity);
	if (worker->local_queue == NULL)
		return -1;
	worker->inbox_queue =
		croutine_mpmc_queue_init(scheduler->config.inbox_queue_capacity);
...
int croutine_worker_enqueue_inbox(struct croutine_worker *worker,
	...
	if (atomic_load_explicit(&worker->scheduler->state,
...
static void croutine_worker_suspend(struct croutine_worker *worker) {
	...
	atomic_store_explicit(&worker->state, CROUTINE_WORKER_SUSPENDING,
						  memory_order_release);
	atomic_store_explicit(&worker->state, CROUTINE_WORKER_SUSPENDED,
						  memory_order_release);
	for (;;) {
		...
		case CROUTINE_SCHEDULER_DESTROYING:
			atomic_store_explicit(&worker->state, CROUTINE_WORKER_EXITING,
								  memory_order_release);
			return;
...
static int croutine_worker_enter_waiting(struct croutine_worker *worker) {
	...
	if (atomic_load_explicit(&scheduler->state, memory_order_acquire) !=
		CROUTINE_SCHEDULER_RUNNING)
		return 0;
	if (atomic_load_explicit(&worker->state, memory_order_acquire) !=
		CROUTINE_WORKER_RUNNING)
	...
	atomic_store_explicit(&worker->state, CROUTINE_WORKER_SOURCE_WAITING,
						  memory_order_release);
	atomic_fetch_add_explicit(&scheduler->waiting_workers, 1,
							  memory_order_release);
	atomic_thread_fence(memory_order_seq_cst);
	return 1;
}
...
static void croutine_worker_leave_waiting(struct croutine_worker *worker) {
	...
	if (atomic_load_explicit(&worker->state, memory_order_acquire) !=
			CROUTINE_WORKER_SOURCE_WAITING ||
		atomic_fetch_sub_explicit(&scheduler->waiting_workers, 1,
...
static size_t croutine_worker_local_space(struct croutine_worker *worker) {
	...
	if (worker == NULL || worker->local_queue == NULL ||
		worker->local_queue->capacity == 0)
		return 0;
...
static void croutine_worker_requeue_main(struct croutine_worker *worker,
										 struct croutine_task *task) {
...
static void
croutine_worker_drain_inbox_snapshot(struct croutine_worker *worker) {
	size_t snapshot;
	...
	for (size_t index = 0; index < snapshot; index++) {
		struct croutine_task *task =
			croutine_mpmc_queue_pop(worker->inbox_queue);
...
static struct croutine_task *
croutine_worker_pop_main(struct croutine_worker *worker) {
	struct croutine_task *task;
...
static struct croutine_task *
croutine_worker_take_main_batch(struct croutine_worker *worker) {
	struct croutine_scheduler *scheduler;
...
static struct croutine_task *
croutine_worker_take_peer_batch(struct croutine_worker *worker) {
	struct croutine_scheduler *scheduler;
	size_t worker_count;
	size_t start;
	...
	if (worker_count <= 8) {
		start = atomic_fetch_add_explicit(&scheduler->scan_index, 1,
	...
	for (size_t step = 0; step < worker_count; step++) {
		...
		for (size_t item_index = 0; item_index < batch; item_index++) {
			struct croutine_task *task =
				croutine_cldeque_steal(victim->local_queue);

			if (task == NULL)
				break;
			if (atomic_load_explicit(&task->state, memory_order_acquire) !=
					CROUTINE_TASK_READY ||
				atomic_load_explicit(&task->worker, memory_order_relaxed) !=
					victim)
				abort();
			atomic_store_explicit(&task->worker, worker,
								  memory_order_relaxed);
			if (direct == NULL) {
				direct = task;
				continue;
...
static void croutine_worker_chain_wake(
...
croutine_worker_next_task(struct croutine_worker *worker) {
	...
	if (worker->queue_check_turns >=
		worker->scheduler->config.queue_check_interval) {
		worker->queue_check_turns = 0;
...
static void croutine_worker_dispatch_task(struct croutine_worker *worker,
										  struct croutine_task *task) {
	if (task == NULL)
		return;
...
void croutine_worker_schedule(void) {
	...
	for (;;) {
		...
		atomic_store_explicit(&worker->state, CROUTINE_WORKER_SEARCHING,
							  memory_order_release);
