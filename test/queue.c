...
static void queue_stress_fail(queue_stress_state *state, const char *message) {
	...
	if (atomic_compare_exchange_strong_explicit(&state->failed, &expected, 1,
...
static int queue_stress_mark(queue_stress_state *state, void *item) {
	...
	seen =
		atomic_exchange_explicit(&state->seen[index], 1, memory_order_acq_rel);
	if (seen != 0) {
	...
	consumed =
		atomic_fetch_add_explicit(&state->consumed, 1, memory_order_acq_rel);
...
static int queue_stress_verify(queue_stress_state *state) {
	...
	CHECK(atomic_load_explicit(&state->consumed, memory_order_acquire) ==
			  state->total,
		  "stress test did not consume all items");

	for (size_t index = 0; index < state->total; index++) {
		CHECK(atomic_load_explicit(&state->seen[index], memory_order_acquire) ==
				  1,
			  "stress test missed an item");
	}
...
static int test_mpmc_queue(void) {
	...
	CHECK(croutine_mpmc_queue_init(0) == NULL,
		  "queue init should reject zero capacity");
	CHECK(croutine_mpmc_queue_init(1) == NULL,
		  "queue init should reject capacity 1");
	CHECK(croutine_mpmc_queue_init(3) == NULL,
		  "queue init should reject odd capacity 3");

	queue = croutine_mpmc_queue_init(4);
	CHECK(queue != NULL, "queue init should succeed for power-of-two capacity");

	CHECK(croutine_mpmc_queue_pop(queue) == NULL,
	...
	CHECK(croutine_mpmc_queue_push(queue, &e) == -1,
		  "push to full queue should fail");

	CHECK(croutine_mpmc_queue_pop(queue) == &a,
		  "pop a should succeed and preserve FIFO order");

	CHECK(croutine_mpmc_queue_push(queue, &e) == 1,
		  "push e should wrap through buffer");
...
static void *mpmc_producer_main(void *arg) {
	...
	while (next < context->last &&
		   atomic_load_explicit(&context->state->failed,
								memory_order_acquire) == 0) {
		if (croutine_mpmc_queue_push(context->queue,
									 &context->state->values[next]) == 1) {
			next++;
		} else {
...
static void *mpmc_consumer_main(void *arg) {
	...
	for (;;) {
		...
		if (atomic_load_explicit(&context->state->failed,
								 memory_order_acquire) != 0)
			break;

		consumed = atomic_load_explicit(&context->state->consumed,
										memory_order_acquire);
		...
		if (atomic_load_explicit(context->producers_done,
								 memory_order_acquire) == MPMC_PRODUCER_COUNT &&
			++idle > QUEUE_STRESS_IDLE_LIMIT) {
			queue_stress_fail(context->state,
							  "mpmc stress test stopped making progress");
			break;
		}
		queue_stress_pause(&pause);
	}
...
static int test_mpmc_queue_concurrent_capacity(size_t capacity,
	...
	CHECK(queue_stress_init(&state, total) == 0,
		  "mpmc stress test should allocate state");
	...
	for (size_t index = 0; index < MPMC_PRODUCER_COUNT; index++) {
		...
		producer_contexts[index].last =
			producer_contexts[index].first + items_per_producer;
		if (pthread_create(&producers[index], NULL, mpmc_producer_main,
						   &producer_contexts[index]) != 0) {
			queue_stress_fail(&state,
							  "mpmc stress test failed to create producer");
			goto out_start;
		}
		producer_count++;
	}
	for (size_t index = 0; index < MPMC_CONSUMER_COUNT; index++) {
		...
		if (pthread_create(&consumers[index], NULL, mpmc_consumer_main,
						   &consumer_contexts[index]) != 0) {
			queue_stress_fail(&state,
							  "mpmc stress test failed to create consumer");
			goto out_start;
		}
		consumer_count++;
	}
...
out_start:
	...
	if (producer_count == MPMC_PRODUCER_COUNT &&
		consumer_count == MPMC_CONSUMER_COUNT &&
...
static int test_mpmc_queue_concurrent(void) {
	...
	for (size_t index = 0; index < sizeof(configs) / sizeof(configs[0]);
		 index++) {
		CHECK(test_mpmc_queue_concurrent_capacity(
				  configs[index].capacity, configs[index].items_per_producer) ==
				  0,
...
static int test_cldeque(void) {
	...
	CHECK(croutine_cldeque_init(0) == NULL,
		  "deque init should reject zero capacity");
	CHECK(croutine_cldeque_init(3) == NULL,
		  "deque init should reject non power-of-two capacity 3");
	CHECK(croutine_cldeque_init(6) == NULL,
		  "deque init should reject non power-of-two capacity 6");
	CHECK(croutine_cldeque_init(1) == NULL,
		  "deque init should reject capacity 1");
	CHECK(croutine_cldeque_len(NULL) == 0,
		  "NULL deque length should be zero");

	deque = croutine_cldeque_init(2);
	CHECK(deque != NULL, "deque init should succeed for power-of-two capacity");
	CHECK(croutine_cldeque_len(deque) == 0,
		  "new deque length should be zero");

	CHECK(croutine_cldeque_pop(deque) == NULL,
		  "pop from empty deque should return NULL");
	CHECK(croutine_cldeque_steal(deque) == NULL,
		  "steal from empty deque should return NULL");
	CHECK(croutine_cldeque_len(deque) == 0,
		  "empty operations should preserve zero length");

	CHECK(croutine_cldeque_push(deque, &a) == 1, "push a should succeed");
	CHECK(croutine_cldeque_len(deque) == 1,
		  "deque length should be one after pushing a");
	CHECK(croutine_cldeque_push(deque, &b) == 1, "push b should succeed");
	CHECK(croutine_cldeque_len(deque) == 2,
		  "deque length should reach capacity");
	CHECK(croutine_cldeque_push(deque, &c) == -1,
		  "push to full deque should fail");
	CHECK(croutine_cldeque_len(deque) == 2,
		  "failed push should preserve deque length");

	CHECK(croutine_cldeque_pop(deque) == &b,
		  "pop should return the newest item (LIFO)");
	CHECK(croutine_cldeque_len(deque) == 1,
		  "pop should decrement deque length");
	CHECK(croutine_cldeque_steal(deque) == &a,
		  "steal should return the oldest item (FIFO)");
	CHECK(croutine_cldeque_len(deque) == 0,
		  "steal should drain the deque");
	CHECK(croutine_cldeque_pop(deque) == NULL,
		  "deque should be empty after pop and steal");
	CHECK(croutine_cldeque_steal(deque) == NULL,
		  "steal from drained deque should return NULL");

	CHECK(croutine_cldeque_push(deque, &c) == 1, "push c should succeed");
	CHECK(croutine_cldeque_steal(deque) == &c,
		  "single element steal should succeed");
	CHECK(croutine_cldeque_push(deque, &d) == 1, "push d should succeed");
	CHECK(croutine_cldeque_pop(deque) == &d,
		  "single element pop should succeed");
	CHECK(croutine_cldeque_pop(deque) == NULL,
		  "deque should be empty after single element rounds");
	...
	CHECK(croutine_cldeque_push(deque, &c) == 1, "push c should succeed");
	CHECK(croutine_cldeque_push(deque, &d) == 1, "push d should succeed");
	CHECK(croutine_cldeque_push(deque, &e) == -1,
		  "push to full deque should fail");
	...
	CHECK(croutine_cldeque_push(deque, &e) == 1,
		  "push e should wrap through buffer");
	CHECK(croutine_cldeque_push(deque, &f) == 1,
		  "push f should wrap through buffer");
	CHECK(croutine_cldeque_push(deque, &g) == -1,
		  "push to refilled deque should fail");
	...
	CHECK(croutine_cldeque_pop(deque) == NULL,
		  "deque should be empty after mixed drains");
	CHECK(croutine_cldeque_steal(deque) == NULL,
		  "steal from drained deque should return NULL");
...
static int test_cldeque_index_wrap(void) {
	...
	CHECK(croutine_cldeque_len(deque) == 0,
		  "wrapped empty deque length should be zero");

	CHECK(croutine_cldeque_push(deque, &a) == 1,
		  "wrap push a should succeed");
	CHECK(croutine_cldeque_push(deque, &b) == 1,
		  "wrap push b should cross UINT64_MAX");
	CHECK(croutine_cldeque_push(deque, &c) == 1,
		  "wrap push c should succeed after index rollover");
	CHECK(croutine_cldeque_push(deque, &d) == 1,
		  "wrap push d should fill deque");
	CHECK(croutine_cldeque_len(deque) == 4,
		  "wrapped full deque length should equal capacity");
	CHECK(croutine_cldeque_push(deque, &e) == -1,
		  "wrapped full deque should reject another push");

	CHECK(croutine_cldeque_steal(deque) == &a,
		  "wrapped steal should return oldest item");
	CHECK(croutine_cldeque_len(deque) == 3,
		  "wrapped steal should decrement length");
	CHECK(croutine_cldeque_pop(deque) == &d,
		  "wrapped pop should return newest item");
	CHECK(croutine_cldeque_len(deque) == 2,
		  "wrapped pop should decrement length");
	CHECK(croutine_cldeque_steal(deque) == &b,
		  "steal should advance head through rollover");
	CHECK(croutine_cldeque_pop(deque) == &c,
		  "pop should drain final wrapped item");
	CHECK(croutine_cldeque_len(deque) == 0,
		  "drained wrapped deque length should be zero");
	CHECK(croutine_cldeque_pop(deque) == NULL,
		  "pop from drained wrapped deque should be empty");
	CHECK(croutine_cldeque_steal(deque) == NULL,
		  "steal from drained wrapped deque should be empty");

	CHECK(croutine_cldeque_push(deque, &e) == 1,
		  "deque should remain reusable after index rollover");
	CHECK(croutine_cldeque_pop(deque) == &e,
		  "reused wrapped deque should preserve item");
	CHECK(croutine_cldeque_len(deque) == 0,
		  "reused wrapped deque should drain cleanly");
...
static void *cldeque_owner_main(void *arg) {
	...
	for (;;) {
		...
		if (next < context->state->total &&
			croutine_cldeque_push(context->deque,
								  &context->state->values[next]) == 1) {
			next++;
			continue;
		}
...
static void *cldeque_thief_main(void *arg) {
	...
	for (;;) {
		...
		if (atomic_load_explicit(&context->state->consumed,
								 memory_order_acquire) >=
			context->state->total)
			break;
		...
		if (atomic_load_explicit(context->owner_done, memory_order_acquire) !=
				0 &&
			++idle > QUEUE_STRESS_IDLE_LIMIT) {
			queue_stress_fail(context->state,
							  "cldeque stress test stopped making progress");
			break;
		}
		queue_stress_pause(&pause);
	}
...
static int test_cldeque_concurrent_capacity(size_t capacity, size_t total,
	...
	CHECK(queue_stress_init(&state, total) == 0,
		  "cldeque stress test should allocate state");
	...
	if (pthread_create(&owner, NULL, cldeque_owner_main, &owner_context) != 0) {
		queue_stress_fail(&state,
						  "cldeque stress test failed to create owner");
	} else {
		owner_started = 1;
	}
	...
	for (size_t index = 0; index < CLDEQUE_THIEF_COUNT; index++) {
		...
		if (pthread_create(&thieves[index], NULL, cldeque_thief_main,
						   &thief_contexts[index]) != 0) {
			queue_stress_fail(&state,
							  "cldeque stress test failed to create thief");
			break;
		}
		thief_count++;
	}
	...
	if (owner_started != 0 && thief_count == CLDEQUE_THIEF_COUNT &&
		queue_stress_verify(&state) == 0)
		ret = 0;
...
static int test_cldeque_concurrent(void) {
	...
	for (size_t index = 0; index < sizeof(configs) / sizeof(configs[0]);
		 index++) {
		CHECK(test_cldeque_concurrent_capacity(
				  configs[index].capacity, configs[index].items,
